...
import tensorflow.compat.v1 as tf  # to solve "no module app" when running "tf.app.run"

import xml.etree.ElementTree as ET

from datasets.dataset_utils import int64_feature, float_feature, bytes_feature
from datasets.lunotim_common import LABELS, IM_EXT
...
def _process_image(directory, name):
    ...
    """
    # Read the image file.
    filename = os.path.join(directory, DIRECTORY_IMAGES, name + '.' + IM_EXT)
    #filename = directory + DIRECTORY_IMAGES + name + '.' + IM_EXT
...
def run(dataset_dir, output_dir, name='voc_train', shuffling=False, samples_per_files=1):
    """Runs the conversion operation.

    Args:
      dataset_dir: The dataset directory where the dataset is stored.
      output_dir: Output directory.
      samples_per_files: Number of samples inside a tfrecord file (default is 1).
    """
